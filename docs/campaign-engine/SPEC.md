...
| D31 | Referrals only; destination is Content's question | The brand wizard's four cards: **Views** (views only), **Content** (stands alone), **Referrals** (a referral objective with no views target) and **Hybrid** (Views and Referrals ticked together: a views target plus a referral budget, as before). Referrals only buys no views: `targetViews` is unset, `budget`, `creatorPool`, `platformFee` and `costPerView` are 0, and the quote and checkout total are just the referral budget (fee inside it, minimum ₦1,000 at checkout; a draft can be saved below it). Payment books only the referral top-up under the checkout reference (no zero views deposit); that booking puts the campaign live and creates its places. Places (`slotCount`, default 5, admin-editable; brands can't set it yet) carry view target 0 and reward 0, so creators are paid only admin's reward per verified conversion through the referral machinery (codes, signed webhook, reservation, hold, back-pay, refunds, reconciliation, statement). Views never complete it; view top-ups are refused (`TOPUP_NOT_FOR_REFERRALS_ONLY`); connect-before-pay applies as for sign-ups. Editing a draft between Hybrid and Referrals sets or clears `targetViews` and reprices. Separately, where content goes (creator page, brand page, both) and its usage rights are only asked for Content in the wizard: every other campaign is stored as creator page, and a Content draft changed to another objective goes back to the standard licence (API clients can still send custom terms, D30) |
| D32 | Social connections that need reconnecting | When Meta refuses a token with OAuthException 190 (any subcode) or 102, or TikTok answers `invalid_grant` / `access_token_invalid` (or the refresh token is missing or expired), the `MetaConnection` / `TikTokConnection` gets `needsReconnect`, `needsReconnectAt` and a short human `needsReconnectReason`, set once (atomically). The view syncs and the daily followers refresh skip flagged connections (other creators keep syncing); connecting again through the existing callbacks clears it. A flagged account still counts as connected for verification and joining. `GET /api/meta/status`, `GET /api/tiktok/status`, the creator dashboard payload and admin **Users & Creators** return the flag. The creator gets one in-app notification (`social_reconnect_needed`) when it's first set, a banner on the creator dashboard with **Reconnect**, and **Needs reconnecting** on Profile → Social accounts. No email (no creator email helper to reuse). Facebook page-token failures aren't detected yet (the video stats call swallows its errors) |
| D33 | Ops alert: creator must reconnect | `social_reconnect_needed` ("Creator Must Reconnect Social Account"), one alert per connection (creator + provider, subject type `connection`), raised while the connection is flagged (D32) and the creator has a post on that platform on a live or paused campaign (posted / verifying, or a verified views-bonus post). Resolves when they reconnect, disconnect or nothing live is left. Reopens after an admin resolve when the number of live posts roughly doubles or after 24 hours, like the other count alerts |
| D34 | One post, one submission | Every path that saves a live post link (views `PATCH /api/submissions/:id/mark-posted`, content campaigns' mark-posted, `POST /api/tiktok/videos/:id/link-submission`) normalises it to `postedPlatforms[].postKey`: `instagram:<shortcode>`, `tiktok:<video id>`, `facebook:<video id>`, else `url:<host without www, lowercased><path without trailing slash>` (query and fragment dropped). A link whose key is on any other submission (any creator, any campaign) is refused with 409 `POST_ALREADY_USED`, "This post is already linked to another campaign. Share a new post made for this campaign."; saving the same link again on the same submission is fine. Rows saved before `postKey` aren't backfilled: the check also matches them on `postUrl` (regex on the shortcode / video id / URL) and `tiktokVideoId`, which is a scan of submissions with post links; fine at today's volume, backfill `postKey` and drop the fallback if it gets slow. TikTok short links (vm/vt) are keyed by their URL, not the video id. Existing duplicates (e.g. the two campaigns sharing one Instagram post) aren't touched; admin resolves them by hand |
| D35 | Posts made before approval don't earn views | When the Instagram sync (media `timestamp`) or TikTok sync (`create_time`) sees a post published before the content was approved (`reviewedAt`; else the placement's `claimedAt`; else when it was submitted), it doesn't update that post's views, sets `postPredatesCampaign` / `postPredatesCampaignAt` on the submission and records a `post_predates_campaign` submission event ("Post made before approval: views not counted") and one log line, once. Nothing is deleted. Views already counted before this shipped stay as they were (not clawed back, since pay may already be booked); new posts stay at 0. Facebook has no publish time in the sync, so it isn't checked |
