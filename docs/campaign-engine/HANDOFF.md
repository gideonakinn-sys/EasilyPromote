...
- **Audience demographics from TikTok/Instagram APIs** ("Verified audience" label). Needs Instagram Graph `instagram_manage_insights` (business/creator accounts: `follower_demographics` by city/country/age/gender) and TikTok Business/Research API access, both requiring app review. `Backend/scripts/checkInstagramApi.js` exists for probing. Build only after approvals: sync job, store `audience.source = "api"`, prefer API data over self-reported, label "Verified audience".
