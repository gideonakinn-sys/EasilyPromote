...
export function CampaignDetailsDrawer({
  ...
  const renderActivity = () => {
    ...
    if (displayCampaign.status === "delivered") {
      items.push(
        <div key="activity-delivered" className="space-y-4 border-b border-neutral-100 pb-5">
          {renderContentCard("delivered", undefined, timelineTime("posted") || displayCampaign.postedAgo)}
        </div>
      );

      items.push(
        <div key="activity-live" className="space-y-4 border-b border-neutral-100 pb-5">
          {renderContentCard("live_tracking", undefined, timelineTime("posted") || displayCampaign.postedAgo)}
        </div>
      );

      items.push(
        <div key="activity-approved" className="space-y-4 border-b border-neutral-100 pb-5">
          {renderContentCard("approved_post", undefined, timelineTime("reviewed") || displayCampaign.reviewedAgo)}
        </div>
      );

      items.push(
        <div key="activity-changes" className="space-y-4 border-b border-neutral-100 pb-5">
              ...
              <div className={cn("grid gap-2.5", isMobile ? "grid-cols-1" : "grid-cols-3")}>
                {linkPlatforms.map((p) => (
                  <input
                    ...
                    className="w-full min-w-0 px-3 py-2.5 bg-white border border-neutral-200 rounded-full text-sm font-medium text-neutral-900 placeholder-neutral-300 focus:outline-none focus:border-neutral-400 font-rethink"
                  />
                ))}
              </div>
              <button
