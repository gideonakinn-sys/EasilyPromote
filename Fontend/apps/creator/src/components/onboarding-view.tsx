...
export function OnboardingView({
  ...
  return (
    <div className="w-full flex flex-col items-center max-w-xl text-center">
      ...
      <h2 data-reveal className="font-motterdam font-normal text-[33px] leading-[42.67px] text-neutral-900 mb-0.5">
        Welcome, {profile.displayName.split(" ")[0]}
      </h2>

      <p data-reveal className="text-sm font-medium text-neutral-500 mb-6 max-w-sm">
        You&apos;re almost ready to start earning. Complete these to unlock your first campaign.
      </p>

      <div data-reveal className="w-[350px] bg-neutral-50 border border-neutral-200 border-dashed rounded-[32px] px-4 py-6 space-y-8 text-left">
        <ChecklistStep
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h4 className="font-rethink font-medium text-sm text-neutral-900 leading-none">{title}</h4>
          <p className="text-xs text-neutral-500 font-medium leading-normal max-w-sm mt-1.5">{description}</p>
        </div>
      </div>
      {!completed && (
        <button
          onClick={onAction}
          className="w-full py-2 bg-white text-neutral-900 font-semibold text-xs border border-neutral-200 rounded-full font-rethink"
        >
          {actionLabel}
        </button>
      )}
