  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl p-6 w-full max-w-sm space-y-4 font-rethink">
        <h3 id="content-action-title" className="font-medium text-base text-stone-900 text-center">
          {title}
        </h3>
        {description && <div className="text-xs text-stone-500 font-medium text-center leading-relaxed">{description}</div>}
        {needsNote && (
          <div className="space-y-1.5">
            <label htmlFor="content-action-note" className="text-xs font-medium text-stone-500">
              {noteLabel}
            </label>
            <textarea
              ...
              maxLength={2000}
              className="w-full px-4 py-3 bg-white border border-stone-200 rounded-xl text-sm font-medium text-stone-900 placeholder-stone-300 focus:outline-none focus:border-stone-400 resize-none min-h-[96px]"
            />
          </div>
        )}
        ...
        <div className="flex gap-3 pt-2">
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 py-2.5 bg-stone-100 text-stone-900 font-semibold text-sm rounded-full"
          >
            Cancel
          </button>
          <button
            ...
            className={cn(
              ...
              tone === "danger" ? "bg-red-50 text-red-600 border-red-200" : "bg-[#FEB604] text-stone-900 border-stone-100"
            )}
          >
