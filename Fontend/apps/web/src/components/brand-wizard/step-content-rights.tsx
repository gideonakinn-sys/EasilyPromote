...
import { ChipGroup, Field, OptionCard, StepHeading, TEXTAREA_CLASS, TEXT_INPUT_CLASS } from "./wizard-fields";
import {
  DESTINATION_OPTIONS,
...
interface StepContentRightsProps {
  data: WizardData;
  update: (patch: Partial<WizardData>) => void;
}

// SPEC D30: the brand's own terms, which creators accept before submitting content.
function CustomTermsForm({ data, update }: StepContentRightsProps) {
    ...
    <div className="bg-white border border-neutral-200 rounded-2xl p-4 space-y-5">
      ...
      <div className="bg-neutral-50 border border-neutral-200 rounded-xl px-3 py-2.5">
        <p className="text-[11px] text-neutral-600 font-medium font-rethink leading-relaxed">
          Creators accept these terms before they submit content. You can change them while the campaign is a draft, but not after it
          launches.
        </p>
      </div>
    </div>
...
// Content campaigns only: where the finished content goes, and how the brand may use it.
export function StepContentRights({ data, update }: StepContentRightsProps) {
  return (
