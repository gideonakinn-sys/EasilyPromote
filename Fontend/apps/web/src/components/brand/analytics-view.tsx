...
const STATUS_BLOCKS = [
  ...
  { key: "drafts", label: "Drafts", color: "bg-stone-300" },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-xs font-medium text-stone-500">How your campaigns are performing.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-stone-100 bg-white p-8 text-center">
          <p className="text-sm font-medium text-stone-900">Analytics didn&apos;t load</p>
        ...
        stats && (
          <>
            ...
            <div className="rounded-2xl border border-stone-100 bg-white p-5">
              <h3 className="text-sm font-semibold text-stone-900">Status breakdown</h3>
                ...
                {STATUS_BLOCKS.map((block) => {
                  ...
                  return (
                    <div key={block.key} className="flex items-center gap-3">
                      <span className="w-24 text-xs font-medium text-stone-500">{block.label}</span>
                      <div className="h-2 flex-1 overflow-hidden rounded-full bg-stone-100">
                        <div className={cn("h-full rounded-full", block.color)} style={{ width: `${pct}%` }} />
                      </div>
                      <span className="w-10 text-right text-xs font-semibold text-stone-900">{count}</span>
                    </div>
                  );
                })}
              </div>
            ...
            {stats.conversions > 0 && (
              <div className="rounded-2xl border border-stone-100 bg-white p-5">
                <h3 className="text-sm font-semibold text-stone-900">Referral conversions</h3>
