    ...
    <div data-reveal className="w-full max-w-lg bg-white border border-neutral-200 rounded-3xl p-8 text-center">
      ...
      <div className="bg-neutral-50 border border-neutral-200/50 rounded-2xl p-4 mb-6 text-left space-y-3">
        <div className="space-y-0.5">
          <span className="text-[10px] font-medium text-neutral-500 block">Withdraw by campaign</span>
        ...
        {!hasBankAccount && (
          <p className="font-rethink text-[11px] text-neutral-500 font-medium">Add your bank account above to withdraw.</p>
        )}
        {withdrawCampaigns.length === 0 ? (
          ...
          <div className="space-y-2.5">
            {withdrawCampaigns.map((c) => {
              ...
              return (
                <div key={c.id} className="bg-white border border-neutral-200/60 rounded-xl p-3 space-y-2">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-rethink text-sm font-medium text-neutral-800 truncate">{c.title}</p>
                      <p className="font-rethink text-xs text-neutral-500">{availableParts(c)}</p>
                    </div>
                    <span className="font-rethink text-sm font-medium text-neutral-900 shrink-0">₦{c.total.toLocaleString()}</span>
                  </div>
                  {(c.onHold ?? []).length > 0 && (
                    <ul className="space-y-0.5">
                      {(c.onHold ?? []).map((hold) => (
                        <li
                          ...
                          className="font-rethink text-[11px] font-medium text-neutral-500 flex justify-between gap-3"
                        >
                          <span>
                  ...
                  {c.payoutDate && ["available", "below_minimum", "nothing_yet"].includes(state) && (
                    <p className="font-rethink text-[11px] font-medium text-neutral-400">
                  ...
                  {state === "available" && (
                    <button
                      ...
                      className="w-full py-2 bg-[#FEB604] text-neutral-950 font-semibold text-xs rounded-full font-rethink disabled:bg-neutral-200 disabled:text-neutral-400"
                    >
                  ...
                  {state === "withdrawn_this_week" && (
                    <p className="font-rethink text-[11px] font-medium text-neutral-500">
      ...
      {viewsByCampaign.length > 0 && (
        <div className="bg-neutral-50 border border-neutral-200/50 rounded-2xl p-4 mb-6 text-left space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-medium text-neutral-500">View earnings by campaign</span>
            {pending > 0 && (
            ...
            {viewsByCampaign.map((c) => (
              <div key={c.id} className="flex items-center justify-between gap-3 text-sm">
                ...
                <div className="text-right shrink-0">
                  <span className="font-rethink font-medium text-neutral-900 block">₦{c.availableToWithdraw.toLocaleString()}</span>
                  <span className="font-rethink text-[10px] text-neutral-500">{c.withdrawable ? "available" : "not yet withdrawable"}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {fixedCampaigns.length > 0 && (
        <div className="bg-neutral-50 border border-neutral-200/50 rounded-2xl p-4 mb-6 text-left space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-medium text-neutral-500">{bonusCampaigns.length > 0 ? "Fixed and Base Pay" : "Fixed Pay"}</span>
            <span className="font-rethink text-sm font-medium text-neutral-900">₦{(fixed?.earned ?? 0).toLocaleString()}</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white border border-neutral-200/60 rounded-xl px-3 py-2">
              <span className="text-[10px] font-medium text-neutral-500 block">On Hold</span>
              <span className="font-rethink text-sm font-medium text-neutral-900">
                ₦{roundKobo((fixed?.awaitingDelivery ?? 0) + (fixed?.onHold ?? 0)).toLocaleString()}
              </span>
            </div>
            <div className="bg-white border border-neutral-200/60 rounded-xl px-3 py-2">
              <span className="text-[10px] font-medium text-neutral-500 block">Withdrawable</span>
          ...
          <div className="space-y-2.5">
            {fixedCampaigns.map((c) => (
              <div key={c.id} className="flex items-center justify-between gap-3 text-sm">
                ...
                <span className="font-rethink font-medium text-neutral-900 shrink-0">₦{c.earned.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {bonusCampaigns.length > 0 && (
        <div className="bg-neutral-50 border border-neutral-200/50 rounded-2xl p-4 mb-6 text-left space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-medium text-neutral-500">Bonus</span>
            <span className="font-rethink text-sm font-medium text-neutral-900">₦{(bonus?.earned ?? 0).toLocaleString()}</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white border border-neutral-200/60 rounded-xl px-3 py-2">
              <span className="text-[10px] font-medium text-neutral-500 block">On Hold ({bonus?.holdDays ?? 7} Days)</span>
              <span className="font-rethink text-sm font-medium text-neutral-900">₦{(bonus?.onHold ?? 0).toLocaleString()}</span>
            </div>
            <div className="bg-white border border-neutral-200/60 rounded-xl px-3 py-2">
              <span className="text-[10px] font-medium text-neutral-500 block">Withdrawable</span>
      ...
      {referralCampaigns.length > 0 && (
        <div className="bg-neutral-50 border border-neutral-200/50 rounded-2xl p-4 mb-6 text-left space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-medium text-neutral-500">Referral earnings</span>
            <span className="font-rethink text-sm font-medium text-neutral-900">₦{(referral?.earned ?? 0).toLocaleString()}</span>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-white border border-neutral-200/60 rounded-xl px-3 py-2">
              <span className="text-[10px] font-medium text-neutral-500 block">On Hold ({referral?.holdDays ?? 7} Days)</span>
              <span className="font-rethink text-sm font-medium text-neutral-900">₦{(referral?.pending ?? 0).toLocaleString()}</span>
            </div>
            <div className="bg-white border border-neutral-200/60 rounded-xl px-3 py-2">
              <span className="text-[10px] font-medium text-neutral-500 block">Withdrawable</span>
            ...
            {withdrawals.map((w) => (
              <div key={w.id} className="bg-neutral-50 border border-neutral-200/50 rounded-xl p-3">
                <div className="flex items-center justify-between text-sm">
                  <span className="font-rethink font-medium text-neutral-800 truncate">
                    {w.campaignName}
                    {w.kind === "referral" && (
                ...
                {w.kind === "campaign" && (
                  <p className="font-rethink text-[11px] text-neutral-500 font-medium mt-0.5">
      ...
      {(appealable.length > 0 || appeals.length > 0) && (
        <div className="mt-6 text-left">
          <span className="text-[10px] font-medium text-neutral-500">Payout Appeals</span>
          <p className="font-rethink text-[11px] text-neutral-400 font-medium mt-0.5">
            If a withdrawal was rejected or pay was removed and you think that&apos;s wrong, you have 7 days to appeal.
          </p>
          <div className="mt-2 space-y-2">
            {appealable.map((item) => (
              <div key={`${item.subjectType}-${item.subjectId}`} className="bg-neutral-50 border border-neutral-200/50 rounded-xl p-3 space-y-1">
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="font-rethink font-medium text-neutral-800 truncate">{item.campaignName}</span>
                  {item.amount > 0 && <span className="font-rethink font-medium text-neutral-900">₦{item.amount.toLocaleString()}</span>}
                </div>
                <p className="font-rethink text-[11px] text-neutral-500 font-medium">
                  {APPEAL_SUBJECT[item.subjectType]}
                  {item.decisionReason && `: ${item.decisionReason}`}
                </p>
                <div className="flex items-center justify-between gap-3 pt-1">
                  <span className="text-[11px] text-neutral-400 font-medium">Appeal by {formatHoldDate(item.appealableUntil)}</span>
                  <button
                    type="button"
                    onClick={() => {
                      setAppealReason("");
                    ...
                    className="px-4 py-1.5 bg-white border border-neutral-200 rounded-full font-rethink font-semibold text-xs text-neutral-900"
                  >
                    Appeal
                  </button>
                </div>
            ...
            {appeals.map((appeal) => (
              <div key={appeal.id} className="bg-neutral-50 border border-neutral-200/50 rounded-xl p-3 space-y-1">
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="font-rethink font-medium text-neutral-800 truncate">{appeal.campaignName}</span>
                  <span
                    className={cn(
                      ...
                      appeal.status === "denied" && "bg-neutral-100 text-neutral-600 border-neutral-200"
                    )}
                  >
                ...
                {appeal.resolutionNote && <p className="font-rethink text-[11px] text-neutral-600 font-medium">{appeal.resolutionNote}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      {appealing && (
        <div
          className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-neutral-950/40 p-4"
        ...
        >
          ...
          >
            <div className="space-y-1">
              <h3 id="appeal-heading" className="font-rethink font-medium text-lg text-neutral-900">
            ...
            <label className="block space-y-1">
              <span className="font-rethink text-[11px] font-medium text-neutral-500">Why should it be reviewed?</span>
              <textarea
                value={appealReason}
                onChange={(e) => setAppealReason(e.target.value)}
                rows={4}
                maxLength={2000}
                className="w-full rounded-xl border border-neutral-200 px-3 py-2 font-rethink text-sm font-medium text-neutral-900 outline-none focus:border-neutral-400"
              />
            </label>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-3 border border-neutral-200 text-neutral-600 font-semibold text-sm rounded-full font-rethink disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={sendingAppeal}
                className="flex-1 py-3 bg-[#FEB604] text-neutral-950 font-semibold text-sm rounded-full font-rethink disabled:bg-neutral-200 disabled:text-neutral-400"
              >
      ...
      {confirmCampaign && (
        ...
        >
          ...
          >
            <div className="space-y-1">
              <h3 id="withdraw-heading" className="font-rethink font-medium text-lg text-neutral-900">
            ...
            <dl className="bg-neutral-50 rounded-2xl p-4 space-y-2 text-sm font-rethink">
              {(confirmCampaign.fixedAvailable ?? 0) > 0 && (
                <div className="flex justify-between gap-3">
                  <dt className="text-neutral-500 font-medium">{isHybrid(confirmCampaign) ? "Base Pay" : "Fixed Pay"}</dt>
                  <dd className="text-neutral-900 font-medium tabular-nums">₦{(confirmCampaign.fixedAvailable ?? 0).toLocaleString()}</dd>
                </div>
              )}
              {(confirmCampaign.bonusAvailable ?? 0) > 0 && (
                <div className="flex justify-between gap-3">
                  <dt className="text-neutral-500 font-medium">Bonus</dt>
                  <dd className="text-neutral-900 font-medium tabular-nums">₦{(confirmCampaign.bonusAvailable ?? 0).toLocaleString()}</dd>
                </div>
              )}
              <div className="flex justify-between gap-3">
                <dt className="text-neutral-500 font-medium">Views earnings</dt>
                <dd className="text-neutral-900 font-medium tabular-nums">₦{confirmCampaign.viewsAvailable.toLocaleString()}</dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-neutral-500 font-medium">Referral earnings</dt>
                <dd className="text-neutral-900 font-medium tabular-nums">₦{confirmCampaign.referralAvailable.toLocaleString()}</dd>
              </div>
              <div className="flex justify-between gap-3 border-t border-neutral-200 pt-2">
                ...
                <dd className="text-neutral-900 font-semibold tabular-nums">₦{confirmCampaign.total.toLocaleString()}</dd>
              </div>
            </dl>
            <div className="flex gap-2">
              <button
