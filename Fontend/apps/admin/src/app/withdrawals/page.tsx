    ...
    <div className="min-h-screen bg-[#fafafa] flex font-rethink">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="pb-6 border-b border-neutral-200 mb-6">
          <h1 className="text-2xl font-bold text-neutral-900 tracking-tight">Withdrawal Requests</h1>
          <p className="text-sm text-neutral-500 mt-1">
            Review creator withdrawals against their campaign targets, then release funds from the campaign's escrow.
          </p>
        </header>

        <div className="bg-white border border-neutral-200/90 rounded-2xl p-5 mb-6">
          <span className="text-xs font-semibold text-neutral-500 block mb-1">Pending Requests</span>
        ...
        <div className="bg-white border border-neutral-200/90 rounded-2xl overflow-hidden">
          <div className="p-4 bg-neutral-50 border-b border-neutral-200">
            <h3 className="font-bold text-sm text-neutral-900">Requests</h3>
          </div>
          ...
          <table className="w-full text-left text-xs text-neutral-700">
            ...
            <tbody className="divide-y divide-neutral-100">
              {loading ? (
                ...
                withdrawals.map((w) => (
                  <tr key={w.id} className="hover:bg-neutral-50/80 transition-colors align-top">
                    <td className="px-6 py-4 font-semibold text-neutral-800">{w.creatorName}</td>

                    <td className="px-6 py-4">
                      <p className="font-semibold text-neutral-800">{w.campaignName}</p>
                      <p className="text-[10px] text-neutral-400 uppercase mt-0.5">{w.campaignStatus}</p>
                    </td>

                    <td className="px-6 py-4 text-neutral-600">{w.brandName}</td>
                    ...
                    <td className="px-6 py-4 font-mono text-neutral-600">{formatCurrency(w.escrowBalance)}</td>

                    <td className="px-6 py-4">
                      <p className="font-bold text-neutral-900">{formatCurrency(w.amount)}</p>
                      {w.kind === "campaign" && (
                        <p className="text-[10px] text-neutral-500 mt-0.5 whitespace-nowrap">
                    ...
                    <td className="px-6 py-4">
                      {w.status === "pending" && !canReview ? (
                        <span className="text-[11px] text-neutral-400">Finance admins review withdrawals</span>
                      ) : w.status === "pending" ? (
                        <div className="space-y-2">
                          <input
                            ...
                            className="w-full border border-neutral-200 rounded-lg px-3 py-1.5 text-xs font-rethink text-neutral-700 outline-none focus:border-neutral-400"
                          />
                        ...
                        <p className="text-[11px] text-neutral-500">{w.adminNotes}</p>
                      ) : (
