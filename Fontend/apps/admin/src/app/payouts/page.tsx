    ...
    <div className="min-h-screen bg-[#fafafa] flex font-rethink">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="pb-6 border-b border-neutral-200 mb-6">
          <h1 className="text-2xl font-bold text-neutral-900 tracking-tight">Payouts & Escrow Ledger</h1>
          <p className="text-sm text-neutral-500 mt-1">Audit escrow fund deposits, creator payouts, and financial transactions</p>
        </header>

        {/* Financial Summary Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
          <div className="bg-white border border-neutral-200/90 rounded-2xl p-6 shadow-sm">
            <span className="text-xs font-semibold text-neutral-500 uppercase tracking-wider block mb-1">Total Escrow Deposited</span>
            <span className="text-2xl font-bold text-neutral-900">{formatCurrency(summary.escrowDeposited)}</span>
          </div>

          <div className="bg-white border border-neutral-200/90 rounded-2xl p-6 shadow-sm">
            <span className="text-xs font-semibold text-neutral-500 uppercase tracking-wider block mb-1">Released Creator Payouts</span>
            <span className="text-2xl font-bold text-green-600">{formatCurrency(summary.releasedPayouts)}</span>
          </div>

          <div className="bg-white border border-neutral-200/90 rounded-2xl p-6 shadow-sm">
            <span className="text-xs font-semibold text-neutral-500 uppercase tracking-wider block mb-1">Brand Refunds Issued</span>
            <span className="text-2xl font-bold text-neutral-900">{formatCurrency(summary.refunds)}</span>
          </div>
        </div>
        ...
        <div className="bg-white border border-neutral-200/90 rounded-2xl shadow-sm overflow-hidden">
          <div className="p-4 bg-neutral-50 border-b border-neutral-200 flex items-center justify-between">
            <h3 className="font-bold text-sm text-neutral-900">Transaction History</h3>
            <span className="text-xs text-neutral-500 font-mono">Immutable Financial Ledger</span>
          </div>
              ...
              {loading ? (
                ...
                transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-neutral-50/80 transition-colors">
                    <td className="px-6 py-4 font-bold text-neutral-900 capitalize">
                      {tx.type.replace("_", " ")}
                    </td>
