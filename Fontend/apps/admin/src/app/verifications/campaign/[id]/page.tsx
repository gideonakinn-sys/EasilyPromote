  ...
  return (
    <div className="min-h-screen bg-[#fafafa] flex font-rethink">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <header className="pt-4 pb-6 border-b border-neutral-200 mb-6">
        ...
        <section className="bg-white border border-neutral-200/90 rounded-2xl shadow-sm overflow-hidden mb-6">
          ...
          <table className="w-full text-left text-xs text-neutral-700">
            <thead className="border-b border-neutral-200 font-bold uppercase tracking-wider text-[10px] text-neutral-500">
              <tr>
              ...
              {loading ? (
                ...
                submissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-neutral-50/80 transition-colors">
                    <td className="px-6 py-3 font-bold text-neutral-900 font-mono">
                      @{sub.creatorHandle}
                    ...
                    <td className="px-6 py-3">
                      <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase font-mono bg-neutral-100 text-neutral-700">
                        {sub.status}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-3 font-mono">
                      ...
                      <span className="ml-1.5 text-[10px] text-neutral-400">{sub.payoutStatus}</span>
                    </td>
        ...
        {/* Activity feed */}
        <section className="bg-white border border-neutral-200/90 rounded-2xl shadow-sm overflow-hidden">
                ...
                {visibleEvents.map((event) => (
                  <li key={event.id} className="ml-6">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-sm font-bold text-neutral-900">{event.label}</p>
                      <span
                    ...
                    {event.reason && (
                      <div className="mt-2 p-3 bg-neutral-50 border border-neutral-200 rounded-xl">
                        <p className="text-xs text-neutral-700 leading-relaxed">{event.reason}</p>
                      </div>
                    )}
        ...
        {previewUrl && (
          <div
            className="fixed inset-0 z-50 bg-neutral-950/60 backdrop-blur-sm flex items-center justify-center p-4"
...
function StatCard({
  ...
  return (
    <div className="bg-white border border-neutral-200/90 rounded-2xl shadow-sm p-4">
      <p className="text-[10px] font-bold uppercase tracking-wider text-neutral-500">{label}</p>
      <p className={`text-sm font-bold text-neutral-900 mt-1 ${mono ? "font-mono uppercase" : ""}`}>
        {value}
      </p>
