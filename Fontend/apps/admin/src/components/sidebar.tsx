...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 flex-shrink-0 bg-neutral-950 text-neutral-300 flex flex-col h-screen sticky top-0 font-rethink border-r border-neutral-800">
      {/* Brand Header */}
      <div className="px-6 py-6 border-b border-neutral-800/80">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#FEB604] flex items-center justify-center shadow-lg shadow-[#FEB604]/20 flex-shrink-0">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor" className="text-neutral-950">
              <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />
            </svg>
          </div>
          <div>
            <span className="font-semibold text-base text-white leading-tight block tracking-tight">EasilyPromote</span>
            <span className="text-[11px] font-medium text-neutral-400 font-mono uppercase tracking-wider">Admin Console</span>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        <div className="px-3 pb-2 text-[10px] font-semibold tracking-wider text-neutral-500 uppercase">Management</div>
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <span className={isActive ? "text-[#FEB604]" : "text-neutral-400"}>{item.icon}</span>
      ...
      <div className="p-4 border-t border-neutral-800/80 bg-neutral-950">
        <div className="flex items-center gap-3 p-2 rounded-xl bg-neutral-900/60 border border-neutral-800 mb-2">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-xs font-semibold text-neutral-100 truncate leading-snug">{user?.name || "Admin User"}</p>
            <p className="text-[10px] text-neutral-400 capitalize truncate font-mono">{user?.role || "Administrator"}</p>
          </div>
        </div>
        <button
